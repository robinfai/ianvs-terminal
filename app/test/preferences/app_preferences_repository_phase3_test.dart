...
void main() {
  test('app preferences repository returns null when preferences file is absent', () async {
  ...
  test('app preferences repository fills missing fields with schema defaults', () async {
    final directory = await Directory.systemTemp.createTemp(
  ...
  test('app preferences repository quarantines corrupt files and repairs defaults', () async {
    final directory = await Directory.systemTemp.createTemp(
    ...
    expect(
      directory
          ...
          .whereType<File>()
          .where(
            (entry) => entry.path.contains('flutterm_preferences.json.corrupt'),
          )
          .length,
      1,
    );
