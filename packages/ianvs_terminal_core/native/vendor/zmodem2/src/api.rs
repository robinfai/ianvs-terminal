...
pub struct FileInfo<'a> {
    ...
    /// Modification time as whole seconds since the Unix epoch, when known.
...
impl<'a> FileInfo<'a> {
    ...
    /// Adds a modification time expressed as whole seconds since the Unix epoch.
    #[must_use]
    pub const fn with_modification_time(mut self, modification_time: u64) -> Self {
...
pub enum Event<'a> {
    ...
    FileCompleted,
    /// The peer declined the current file with `ZSKIP`.
    FileSkipped,
