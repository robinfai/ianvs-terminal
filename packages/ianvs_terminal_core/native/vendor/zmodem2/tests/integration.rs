...
fn create_test_file(path: &Path, size_bytes: usize, seed: u8) {
    let mut file = File::create(path).unwrap();
    let mut buffer = [0u8; 1024];
    for (i, byte) in buffer.iter_mut().enumerate() {
        *byte = ((i * 31 + usize::from(seed) * 17) % 256) as u8;
    }
...
fn set_file_mtime(path: &Path, seconds: u64) {
...
fn assert_files_equal(path1: &Path, path2: &Path) {
    ...
    assert_eq!(
        file_mtime_seconds(path1),
        file_mtime_seconds(path2),
...
impl TestFiles {
    fn new() -> Self {
        ...
        for i in 0..FILE_COUNT {
            ...
            // Every member is deliberately distinct in content, length and
            // whole-second mtime so a batch mix-up cannot accidentally pass.
            create_test_file(&path, FILE_SIZE + i * 137, i as u8);
            set_file_mtime(&path, 1_700_000_000 + (i as u64 * 97));
            paths.push(path);
        }
        Self { dir, paths }
    }
...
fn setup_sz(test_files: &TestFiles) -> (Child, MockPort<ChildStdout, ChildStdin>) {
    let mut sz_process = Command::new(env!("ZMODEM_SZ_BIN"))
        // Exercise GNU lrzsz's largest optional subpacket size together with
        // ESCCTL. This is the same nonstop-I/O receive path used by Ianvs.
        .args(["--start-8k", "-e"])
...
fn setup_rz(dest_dir: &TempDir) -> (Child, MockPort<ChildStdout, ChildStdin>) {
    let mut rz_process: Child = Command::new(env!("ZMODEM_RZ_BIN"))
        // Match terminal integrations such as `rz -bye`: GNU rz advertises
        // ESCCTL with `-e`, so sender data and CRC quoting are exercised too.
        .arg("-e")
        .stdout(Stdio::piped())
        .stdin(Stdio::piped())
            ...
            Action::Event(event) => {
                ...
                match event {
                    Event::FileStarted(info) => {
                        ...
                        current_file_mtime = info.modification_time;
                    }
                    Event::FileCompleted => {
                        let modification_time = current_file_mtime
                            .take()
                            .expect("lrzsz should advertise a file modification time");
                        let received = open_files
                            .get(&current_file_name_bytes)
                            .expect("completed file should still be open");
                        received
                            .file
                            .set_times(
                                FileTimes::new().set_modified(
                                    UNIX_EPOCH + Duration::from_secs(modification_time),
                                ),
                            )
                            .unwrap();
                    }
...
fn test_batch_to_rz() {
    ...
    sender
        .start_file(
            FileInfo::new(first_filename.as_bytes(), Some(Position::new(first_size)))
                .with_modification_time(file_mtime_seconds(first_path)),
        )
        .unwrap();
                        ...
                        if let Some(next_path) = file_iter.next() {
                            ...
                            sender
                                .start_file(
                                    ...
                                    .with_modification_time(file_mtime_seconds(next_path)),
                                )
                                .unwrap();
