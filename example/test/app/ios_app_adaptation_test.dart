...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pump();
      expect(nativeBackend.writes, contains(orderedEquals([0x03])));
      await tester.tap(find.byKey(const Key('ios-terminal-more')));
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const Key('ios-terminal-key-Control A')));
      await tester.pump();
      expect(nativeBackend.writes, contains(orderedEquals([0x01])));
