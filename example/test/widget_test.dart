...
void main() {
  ...
  testWidgets('action search can open paste history without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets(
    'action search can move to next search match without shell write',
    (tester) async {
      final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open copy mode without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open captured output without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open annotations without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open shell integration without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open tmux integration without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open coprocess without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open password manager without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can open instant replay without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
    const windowBridgeChannel = MethodChannel('app/window_bridge');
  ...
  testWidgets('action search can copy block output without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
    String? copiedText;
  ...
  testWidgets('action search can save block output without shell write', (
    tester,
  ) async {
    final previousPathProvider = PathProviderPlatform.instance;
    final supportDirectory = Directory.systemTemp.createTempSync(
      'ianvs-block-output-widget-test-',
    );
    ...
    expect(exportFiles.single.readAsStringSync(), 'warning: one\nerror: two');
    expect(fakeBindings.writes, isEmpty);
  });
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      fakeBindings.setFrame(1, {
        'rows': [
          {'index': 10, 'text': r'$ make', 'style_runs': const []},
          {'index': 11, 'text': 'needle in block', 'style_runs': const []},
          {'index': 12, 'text': 'error: two', 'style_runs': const []},
          {'index': 13, 'text': r'$ ', 'style_runs': const []},
        ],
        'cursor': {'row': 13, 'col': 2, 'visible': true},
      ...
      fakeBindings.setSearchMatches(1, 'needle', [
        ...
        {'row': 20, 'start_col': 0, 'end_col': 6, 'text': 'newer needle'},
      ]);
      fakeBindings.enqueueEvent(
        1,
        PtyEvent(
  ...
  testWidgets('action search can reinput and rerun an active command block', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets(
    'action search can toggle bell notifications without shell write',
    (tester) async {
      final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can toggle activity monitor without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can split right without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can split down without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can focus previous pane without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can resize pane without shell write', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
    ...
    final pane1 = find.byKey(const Key('shell-pane-1'));
    final pane2 = find.byKey(const Key('shell-pane-2'));
    final pane1WidthBefore = tester.getSize(pane1).width;
    final pane2WidthBefore = tester.getSize(pane2).width;
  ...
  testWidgets('action search can duplicate current cwd into new tab', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
    ...
    expect(find.bySemanticsIdentifier('shell-tab-2'), findsOneWidget);
    _expectSelectedTab(tester, '2');
    expect(fakeBindings.writes.map(utf8.decode).toList(), ['cd /tmp/project']);
  });
  ...
  testWidgets(
    'action search can explain unavailable recent directory without shell write',
    (tester) async {
      final fakeBindings = FakePtyBackend();
  ...
  testWidgets('action search can explain unavailable diagnostics export', (
    tester,
  ) async {
    final fakeBindings = FakePtyBackend();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      instantReplayStore.record(
        '1',
        _replayFrameWithRows({11: 'selected review line'}),
      );
      ...
      fakeBindings.setFrame(1, {
        'rows': [
          {'index': 10, 'text': r'$ false', 'style_runs': const []},
          {'index': 11, 'text': 'selected review line', 'style_runs': const []},
          {'index': 12, 'text': r'$ ', 'style_runs': const []},
        ],
        'cursor': {'row': 12, 'col': 2, 'visible': true},
        'selection': null,
      ...
      expect(find.byKey(const Key('instant-replay-workspace')), findsOneWidget);
      expect(find.textContaining('Review: false'), findsOneWidget);
      final timeline = tester.widget<Slider>(
        find.byKey(const Key('instant-replay-timeline')),
      );
      expect(timeline.value, timeline.max);
