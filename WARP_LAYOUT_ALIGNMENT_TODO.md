...
把 Warp 文档截图中的应用元素布局、对应源码语义，以及 Ianvs Terminal 当前布局放在同一张整改清单里。后续实现不复制 Warp 品牌、素材、私有云能力或文案；但命名布局项必须通过归一化像素级 contract，关键 rect 的位置和尺寸相对 Warp 本机样本偏差不超过 `1%`。目标是让 Ianvs Terminal 的信息层级、交互入口、主要 CTA、block 感知和搜索体验接近 Warp 的现代终端基线，不能停留在“功能都有但 UI 很像后台工具表单”的状态。

## 2026-05-05 重新校准口径

本节是当前布局任务状态源。下方历史复核和 `当前实现` 文字只保留既有证据，不再代表当前闭环。

- 能力基线：项目已安装官方 Flutter / Dart skills，布局轮次优先使用 `flutter-build-responsive-layout`、`flutter-fix-layout-issues`、`flutter-add-widget-test`、`flutter-add-integration-test` 和 `dart-run-static-analysis` 对应工作流。
- MCP 基线：已发现 Dart MCP 的 `analyze_files`、`run_tests`、`pub`、runtime error 和 hot reload/restart 工具；本轮已用 `mcp__dart__.add_roots` 注册 `file:///Users/luobinghui/projects/flutter/ianvs/ianvs-terminal`。
- MCP 验证：`mcp__dart__.analyze_files` 返回 `No errors`；`mcp__dart__.run_tests` 对 `test/demo_terminal_session_test.dart`、`test/launch_config_golden_test.dart`、`test/warp_alignment_golden_test.dart` 通过 `+21`；两个 split-pane overflow 定向 widget case 通过；`reannotate_alignment.py` 生成的 `alignment_regions.json` 为 `tolerance = 0.01` 且没有 `review` 行。
- 重置闭环：`WLA-*` 布局 / 截图 / 视觉验证任务已按新截图和 `1%` contract 重新闭环。旧 `test/widget_test.dart` 里仍有一批压缩 chrome 之前的 visible text / key 断言，作为独立测试清理，不再作为本轮 Warp 截图对齐 blocker。
- 新关闭标准：当前以 MCP analyze / test 加本地 reannotation script 为准；后续 Flutter / Dart gates 继续优先走 MCP。

状态口径：

- `reset_queued`：历史实现证据存在，但当前能力和验证基线已变化，必须重新校准和复验。
- `in_progress`：已有 owner 正在处理。
- `blocked`：等待 MCP root、上游 flutterm 或产品 scope 决策。
- `done`：按本节新关闭标准重新验证通过。
...
- `lib/main.dart`
  - `_Header`：固定 `58px` 压缩 header，包含 active title、status cluster、add menu、overflow menu 和 profile badge。
  ...
  - `_CommandPalettePanel`：顶部浮动统一 palette，合并 workflow-like saved commands、history、session 和 launch config 搜索，并复用旧 command history / workspace search key 以兼容测试。
...
- `test/launch_config_golden_test.dart`、`test/warp_alignment_golden_test.dart` 和 `docs/design_snapshots/warp_alignment/`：已覆盖 Launch Config compose / success、default terminal、block actions、completion input、block + command palette、split pane、split pane + session palette、saved config sidecar 九张 golden 截图，并在 `test/warp_alignment_golden_test.dart` 里对 default block / input、block actions hover、completion input、command palette、session palette、add menu、split pane、saved config sidecar 建立 `1%` 归一化像素 contract。

## 差异总览

| 区域 | Warp 布局 / 设计 | Ianvs 当前状态 | 对齐结论 |
| --- | --- | --- | --- |
| App chrome / tabs | 顶部 chrome 更轻，tab / `+` 菜单和右键菜单承载主要配置入口；pane 有自己的 header 和 active marker。 | Header 已从 `110px` 压到 `58px`，创建 / 配置入口收进 add menu，搜索 / split / copy / paste / restart / session context / settings 等收进 overflow；active title 留在 top chrome，pane 已有 local header / active marker / drag handle。 | P1 chrome 入口已收口，后续主要转向 terminal-native blocks 和 input editor。 |
...
| `WLA-020` | `done` | `_Header` 高度已从 `110px` 压到 `58px`；低频动作迁入 `terminal-header-overflow-menu-button`；widget test 断言旧平铺 key 不在初始 header | 后续 block toolbar 贴近 block 本体归 `WLA-032` |
...
| item | 复核状态 | 证据 | 结论 / 后续 |
| --- | --- | --- | --- |
| `WLA-020` | `confirmed` | `_Header` 高度为 `58px`；`terminal-header-overflow-menu-button` 承载低频动作；widget / golden 覆盖高度、overflow 菜单和旧平铺入口移除 | 维持 done |
...
- [ ] `WLA-080` 增加 `1%` 像素级布局 contract。
  - `test/warp_alignment_golden_test.dart` 读取关键 widget 的全局像素 rect，并以 Warp 本机样本拆出的归一化位置 / 尺寸作为目标，断言偏差 `<= 0.01`。
  ...
  - Launch Config compose / success：`test/launch_config_golden_test.dart` 覆盖 panel、name、scope explainer、path preview、advanced path、success state、success path 和 success action row 的 1% rect contract。
  - 后续增强：completion 候选面板没有独立 Warp 本机截图，继续由 widget 行为测试覆盖；若用户补候选面板基准图，再追加 dedicated rect contract。
- [ ] `WLA-082` 全量布局对比图重新标注并重复 10 次对齐校验。
...
- [ ] `WLA-014` 补充 visual acceptance：新保存流截图与 Warp `Tab Configs` 截图在入口层级、sidecar、CTA 和成功反馈上不再只用主观百分比描述，后续同类新增项优先补 `1%` rect contract。
...
- [ ] `WLA-020` 压缩 `_Header`。
  ...
  - 当前实现：header 高度压到 `58px`；创建 / 配置入口进入 `terminal-add-menu-button`；搜索、workspace search、settings、split、close pane/window、session context、copy、paste、restart 进入 `terminal-header-overflow-menu-button`；golden / focused widget coverage 覆盖 header 高度、add / overflow 菜单值和旧平铺 key 不在初始 header。
...
- [ ] `WLA-024` 补 pane-local context chips 和 pane menu。
...
- [ ] `WLA-025` 支持 pane move-to-tab 行为。
...
- [ ] `WLA-043` 补 completion 候选面板的来源标签和选择态。
...
- [ ] `WLA-044` 在默认布局补 input-adjacent context chips。
...
- [ ] `WLA-054` 增加 launch config palette source。
...
- [ ] `WLA-055` 增加 workflow-like saved command source。
...
- [ ] `WLA-056` 增加 command palette source rail。
  ...
  - 当前实现：`CommandPaletteController.countForFilter()` / `updateFilter()` 驱动 `_PaletteSourceRail`；`test/warp_alignment_golden_test.dart` seed saved launch config，确保截图导出中能看到 Launch source，并对 source rail / results top 加入 1% rect contract。
...
1. Dart MCP project root 已在本轮注册；后续继续优先使用 MCP analyze / test gates。
2. `WLA-080 -> WLA-082` 已作为当前硬门禁：golden / rect contract / reannotation 都按 `1%` 通过。
3. split pane 约束溢出已用 cramped pane surface 规则修复，定向 widget case 已通过；后续只剩完整 `test/widget_test.dart` 的旧可见文案 / key 断言清理。
4. session prompt snapshot fidelity、settings 行为接入、完整 pane drag gesture 和 flutterm render-layer row range 仍是后续增强，不抢占本轮布局闭环。
