...
只读复审还发现较少见的启动依赖恢复场景：凭据存储暂不可读导致 transport 未创建时，“立即同步”不能自行重建它。这与普通 API 离线不同，不能以本轮网络恢复通过代替该场景。后续已补齐重建入口、并发和退出清理，独立结果见 [启动依赖恢复验收](../sync-transport-retry-20260910/README.md)。
