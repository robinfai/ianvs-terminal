...
- 完整 `make verify` 仍受 `native/core/src/shell_bootstrap.rs` 的两处 `collapsible_if`、一处 `type_complexity` 与 `native/core/src/ssh.rs` 的 `too_many_arguments` 阻断，未放宽 `-D warnings`。
