...
Data API 密钥轮换是独立的长期客户端安全里程碑，当前版本明确不支持。进入实现前必须满足
[ADR-0004](DECISIONS/ADR-0004-client-side-sensitive-encryption.md) 的全量敏感资源可恢复重加密、
并发写入隔离、失败回滚和版本化 KDF 兼容要求。服务端始终只保存不透明信封，
不得增加 verifier 或密钥验证接口来伪装成轮换。
