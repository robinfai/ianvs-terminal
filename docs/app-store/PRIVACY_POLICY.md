...
The app may process terminal input and output, terminal session recordings, SSH host details, usernames, connection preferences, and optional remote data-service settings. Authentication secrets and encryption keys are stored using the operating system credential vault. Other app data is stored in the app container unless the user chooses a remote destination.
...
Ianvs Terminal 是终端与 SSH 客户端。iOS App 可能在设备上处理终端输入与输出、会话记录、SSH 主机信息、用户名、连接偏好以及可选的远程数据服务配置。认证凭据和加密密钥使用系统凭据保险库存储，其他数据默认保存在 App 沙盒中。
