...
- `default_terminal_layout.png` 现在以用户补充的 1656 x 2064 默认显示截图为基准：顶部 chrome、空白 terminal viewport、底部可见 scrollback、input context chips 和 input editor 按 `analysis/default_display_view_annotation.json` 做 1:1 rect contract；单 pane 默认态不再显示 pane-local header 或 block rail 浮层。
