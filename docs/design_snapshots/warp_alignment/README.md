...
- `analysis/reannotated/` 对全部 10 张当前 Ianvs 布局对比图逐一重新标注：default display、block actions、completion input、blocks command palette、split pane、split session palette、add menu、saved config sidecar、launch config compose 和 launch config success。`alignment_regions.json` 保存每个 rect 和归一化 delta；`alignment_iteration_report.md` 记录 10 次重复生成 fingerprint 全部一致，并且 30 个可比 region 均在 `<= 0.01` 对齐阈值内。Launch Config compose / success 没有本地 Warp modal 截图，因此用 Warp `Tab Configs` 文档与 `launch_configs/save_modal.rs` 语义做右侧 Ianvs 标注和说明。
