# T-353 Command Action Search Activity Monitor Dispatch

## Goal

让 action search 能执行高频非写入入口 `activity monitor`，从 action search 直接切换活动监控通知偏好，同时保持动作本身不写 shell。

## Scope

- 从 action search 搜索并选择 `activity monitor` 时复用现有通知偏好切换行为。
- 为 `toggleActivityMonitor` 增加 action search 别名，匹配命令菜单里的用户可见文案。
...
- 不改变 activity detection、系统通知权限或通知发送逻辑。
- 不改变 command-finished notifications 或 bell notifications 行为。
...
- action search 搜索 `activity monitor` 后按 `Enter` 会切换 activity monitor 偏好。
- 执行后显示与命令菜单一致的保存反馈。
- 执行动作不向 shell 写入。
- 命令菜单里的 activity monitor toggle 行为仍沿用既有标签和反馈。
...
- 打开 action search，搜索 `activity monitor`，按 `Enter`。
- 确认 snackbar 显示 activity monitor 已启用或禁用并保存。
- 再次打开命令菜单，确认对应 label 反映新的开关状态。
- 确认执行该动作不写 shell，也不会把 overlay 文案送进终端。

## Done When

- Activity monitor toggle 可以从 action search 执行。
- action search 对 activity monitor toggle 有 widget regression。
...
- 后续仍需考虑把 notification toggle 的 UI feedback 抽成共享 helper，降低重复。
