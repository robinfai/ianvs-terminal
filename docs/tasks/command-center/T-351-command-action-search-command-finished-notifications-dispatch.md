# T-351 Command Action Search Command-Finished Notifications Dispatch

## Goal

让 action search 能执行高频非写入入口 `command-finished notifications`，从 action search 直接切换命令完成通知偏好，同时保持动作本身不写 shell。

## Scope

- 从 action search 搜索并选择 `command finished notifications` 时复用现有通知偏好切换行为。
- 为 `toggleCommandFinishedNotify` 增加 action search 别名，匹配命令菜单里的用户可见文案。
...
- 不改变通知权限、系统通知发送或 shell hook 完成检测逻辑。
- 不改变 bell notifications 或 activity monitor 行为。
...
- action search 搜索 `command finished notifications` 后按 `Enter` 会切换 command-finished notification 偏好。
- 执行后显示与命令菜单一致的保存反馈。
- 执行动作不向 shell 写入。
- 命令菜单里的 notification toggle 行为仍沿用既有标签和反馈。
...
- 打开 action search，搜索 `command finished notifications`，按 `Enter`。
- 确认 snackbar 显示 command-finished notifications 已启用或禁用并保存。
- 再次打开命令菜单，确认对应 label 反映新的开关状态。
- 确认执行该动作不写 shell，也不会把 overlay 文案送进终端。

## Done When

- Command-finished notification toggle 可以从 action search 执行。
- action search 对 command-finished notification toggle 有 widget regression。
...
- 后续可用同一模式接入 bell notifications 和 activity monitor。
