...
func (s *Service) Logout(ctx context.Context, rawToken string) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where(map[string]any{"key": sessionMetadataKey(token.OperationHash)}).Delete(&model.Setting{}).Error; err != nil {
			return err
		}
...
func cleanupExpiredAuthState(db *gorm.DB, now time.Time) error {
	...
	for _, op := range expired {
		if err := db.Where(map[string]any{"key": sessionMetadataKey(op.OperationHash)}).Delete(&model.Setting{}).Error; err != nil {
			return err
		}
	}
