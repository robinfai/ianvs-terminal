...
impl EscapeOptions {
    pub(crate) const fn new(ctrl: bool, bit8: bool) -> Self {
        Self { ctrl, bit8 }
    }
}
...
impl Header {
    ...
    where
        P: Write + ?Sized,
    {
        self.write_with_escape_options(port, false, false)
    }

    pub(crate) fn write_with_escape_options<P>(
        self,
        port: &mut P,
        escape_ctrl: bool,
        escape_8bit: bool,
        ...
        P: Write + ?Sized,
    {
        ...
        if self.encoding == Encoding::ZHEX {
            ...
            if write_slice_escaped_with_escape_options(port, hex, escape_ctrl, escape_8bit)?
                .is_none()
            {
                return Ok(None);
            }
        ...
        } else if write_slice_escaped_with_escape_options(port, &out, escape_ctrl, escape_8bit)?
            .is_none()
        {
            return Ok(None);
        }
...
pub(crate) fn write_slice_escaped_with_escape_options<P>(
    port: &mut P,
    buf: &[u8],
    escape_ctrl: bool,
    escape_8bit: bool,
...
{
    for value in buf {
        if write_byte_escaped_with_escape_options(port, *value, escape_ctrl, escape_8bit)?.is_none()
        {
            return Ok(None);
        }
    }
...
pub(crate) fn write_byte_escaped_with_escape_options<P>(
    port: &mut P,
    value: u8,
    escape_ctrl: bool,
    escape_8bit: bool,
...
where
    P: Write + ?Sized,
{
    let is_control = value & 0x60 == 0;
    let escape_all_control = escape_ctrl && is_control;
    // GNU lrzsz's decoder can invert bit 6 for parity-marked controls
    // (0x80..=0x9f), but ZMODEM defines no reversible ZDLE representation for
    // arbitrary 0xa0..=0xfe bytes. ESC8 therefore quotes the interoperable
    // high-control range and leaves high printable bytes untouched. The fixed
    // ZDLE table continues to provide the standard special case for 0xff.
    let escape_parity_control = escape_8bit && value & 0x80 != 0 && is_control;
    let escaped = if escape_all_control || escape_parity_control {
        value ^ 0x40
    } else {
        zdle::ZDLE_TABLE[value as usize]
    };
    if (escape_all_control || escape_parity_control || escaped != value)
        && port.write_byte(ZDLE)?.is_none()
    {
        return Ok(None);
    }
