...
pub use product_requests::request_session;
#[cfg(test)]
use product_requests::valid_sftp_path;
...
const MAX_PENDING_SFTP_DIRECTORY_JOBS: usize = 16;
const MAX_PENDING_SFTP_OPERATION_JOBS: usize = 16;
...
pub struct TerminalSession {
    ...
    sftp_directory_jobs:
        Mutex<HashMap<u64, tokio::sync::oneshot::Receiver<crate::ssh::SftpDirectoryResult>>>,
    sftp_operation_jobs:
        Mutex<HashMap<u64, tokio::sync::oneshot::Receiver<crate::ssh::SftpOperationResult>>>,
...
impl TerminalSession {
    ...
    fn new_with_ssh_auth(
        ...
        Arc::new(Self {
            ...
            sftp_directory_jobs: Mutex::new(HashMap::new()),
            sftp_operation_jobs: Mutex::new(HashMap::new()),
    ...
    pub fn resize_with_cell_size(
        ...
        let iterm_graphics_before_resize = self.iterm_graphics_to_preserve_for_resize(&state);
        ...
        let resize_guard_graphics =
            restore_iterm_graphics_after_resize(&mut state.terminal, iterm_graphics_before_resize);
    ...
    fn iterm_graphics_to_preserve_for_resize(&self, state: &TerminalState) -> Vec<TerminalGraphic> {
        let mut graphics = state
            ...
            .iter()
            .chain(state.terminal.all_graphics().iter())
            .filter(|graphic| graphic.protocol.as_str() == "iterm")
            .cloned()
            .collect::<Vec<_>>();
        let guard = self.resize_iterm_redraw_guard.lock();
        // The deadline protects only immediate SIGWINCH redraw output, while
        // the retained entries also bridge a resize where a clipped placement
        // is temporarily absent from the current terminal stores.
        for candidate in guard.graphics.iter().cloned() {
            if !graphics
                .iter()
    ...
    fn reconcile_resize_iterm_redraw_guard(&self, terminal: &Terminal) {
        ...
        guard.graphics.retain_mut(|protected| {
            ...
            let locked_display_size_px = protected.locked_iterm_display_size_px();
            *protected = candidate.clone();
            protected.display_size_px = locked_display_size_px;
            true
        });
    ...
    fn start_sftp_directory_listing(&self, path: String) -> Result<u64, SessionError> {
        ...
        let client = self
            .ssh_sftp
            .as_ref()
            .ok_or_else(|| SessionError::Sftp("session does not support SFTP".to_string()))?;
        let mut jobs = self.sftp_directory_jobs.lock();
        if jobs.len() >= MAX_PENDING_SFTP_DIRECTORY_JOBS {
            return Err(SessionError::Sftp(
                "too many pending directory requests".to_string(),
            ));
        }
        let receiver = client
        ...
        let job_id = self
            ...
            .fetch_add(1, Ordering::Relaxed)
            .saturating_add(1);
        if job_id == 0 || jobs.contains_key(&job_id) {
            return Err(SessionError::Sftp(
                "directory request id space exhausted".to_string(),
            ));
        }
        jobs.insert(job_id, receiver);
        Ok(job_id)
    }

    fn poll_sftp_directory_listing(&self, job_id: u64) -> Result<serde_json::Value, SessionError> {
        let mut jobs = self.sftp_directory_jobs.lock();
        let result = jobs
            .get_mut(&job_id)
            .ok_or_else(|| SessionError::Sftp("unknown directory request".to_string()))?
            .try_recv();
        match result {
            Ok(Ok(listing)) => {
                jobs.remove(&job_id);
    ...
    fn cancel_sftp_directory_listing(&self, job_id: u64) -> bool {
        self.sftp_directory_jobs.lock().remove(&job_id).is_some()
    }
    ...
    fn start_sftp_operation(
        ...
        let client = self
            ...
            .as_ref()
            .ok_or_else(|| SessionError::Sftp("session does not support SFTP".to_string()))?;
        let mut jobs = self.sftp_operation_jobs.lock();
        if jobs.len() >= MAX_PENDING_SFTP_OPERATION_JOBS {
            return Err(SessionError::Sftp(
                "too many pending SFTP file requests".to_string(),
            ));
        }
        let receiver = client
        ...
        if job_id == 0 || jobs.contains_key(&job_id) {
            return Err(SessionError::Sftp(
                "SFTP request id space exhausted".to_string(),
            ));
        }
        jobs.insert(job_id, receiver);
        Ok(job_id)
    ...
    fn poll_sftp_operation(&self, job_id: u64) -> Result<serde_json::Value, SessionError> {
        let mut jobs = self.sftp_operation_jobs.lock();
        let result = jobs
            .get_mut(&job_id)
            .ok_or_else(|| SessionError::Sftp("unknown SFTP file request".to_string()))?
            .try_recv();
        match result {
            Ok(Ok(())) => {
                jobs.remove(&job_id);
    ...
    fn cancel_sftp_operation(&self, job_id: u64) -> bool {
        self.sftp_operation_jobs.lock().remove(&job_id).is_some()
    }
...
fn restore_iterm_graphics_after_resize(
    ...
    let mut matched_active = HashSet::new();
    let mut matched_scrollback = HashSet::new();
        ...
        if let Some(index) =
            terminal
                ...
                .find_map(|(index, candidate)| {
                    (!matched_active.contains(&index)
                        && same_iterm_graphic_asset(candidate, &graphic))
                    .then_some(index)
                })
        {
            matched_active.insert(index);
            ...
            .enumerate()
            .find_map(|(index, candidate)| {
                (!matched_scrollback.contains(&index)
                    && same_iterm_graphic_asset(candidate, &graphic))
                .then_some(index)
            })
        {
            matched_scrollback.insert(index);
            ...
            // Retain its terminal-row anchor and original row reservation until
            // a later replay can rebuild the same content.
...
fn apply_locked_iterm_geometry(candidate: &mut TerminalGraphic, locked: &TerminalGraphic) {
    // Keep the thumbnail's rendered pixels stable. The candidate's current
    // cell dimensions and span are authoritative because font zoom changes
    // how many terminal rows those fixed pixels need to reserve.
    candidate.display_size_px = locked.locked_iterm_display_size_px();
}
...
mod tests {
    ...
    #[test]
    fn sftp_paths_require_bounded_absolute_utf8_without_nul() {
        assert!(valid_sftp_path("/"));
        assert!(valid_sftp_path("/srv/项目"));
        assert!(!valid_sftp_path("relative"));
        assert!(!valid_sftp_path("/bad\0path"));
        assert!(!valid_sftp_path(&format!("/{}", "x".repeat(4096))));
    }
