...
- Workspace roots are declared in root `pubspec.yaml`: `example`, `packages/ianvs_pty`, `packages/ianvs_terminal`.
...
- Primary runtime stack flows downward: `example` app shell -> `ianvs_terminal` Dart runtime/viewport -> `ianvs_pty` Dart FFI backend -> `native/core` Rust PTY/VT core.
