...
- Keep shared terminal capability in packages, not in `example`: `ianvs_pty` owns PTY transport/FFI; `ianvs_terminal` owns runtime config, input, viewport, selection, scroll/focus adapters.
...
- Do not expose app vocabulary such as Profile/tab/window chrome from `ianvs_pty`; do not expose app `Profile` from `ianvs_terminal`.
- `ianvs_terminal` internally adapts neutral `TerminalSessionConfig` into the older native wire shape needed by `native/core`; do not push that compatibility detail into app code.
