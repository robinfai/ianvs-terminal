...
- Example app checks: `cd example && flutter analyze`; `cd example && flutter test`; `cd example && flutter test -d macos integration_test/ianvs_smoke_test.dart`.
- Terminal package checks: `cd packages/ianvs_terminal && flutter test`.
- PTY package checks: `cd packages/ianvs_pty && dart test`.
