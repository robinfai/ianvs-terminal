...
- Native/PTY changes: run `cd native/core && cargo fmt --check && cargo test`; run `cd packages/flutterm_pty && dart test` when Dart FFI/backend code is touched.
- `packages/flutterm_terminal` changes: run `cd packages/flutterm_terminal && flutter test`.
...
- macOS smoke currently needs explicit device: `cd example && flutter test -d macos integration_test/flutterm_smoke_test.dart`; unspecified device discovery may hang on Android adb discovery.
