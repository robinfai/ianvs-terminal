...
import { completeWithWasm } from './wasm_engine.mjs';

const defaultHost = '127.0.0.1';
const defaultPort = 17383;
  ...
  return http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && request.url === '/health') {
      writeJson(response, 200, {
        ok: true,
        service: 'ianvs-fig-completion-wasm-service',
      });
      return;
    }
    ...
    try {
      ...
      writeJson(response, 200, await completeWithWasm(body));
    } catch (error) {
...
export function startServer({ host = defaultHost, port = defaultPort } = {}) {
  ...
  server.listen(port, host, () => {
    ...
    console.log(
      `ianvs fig completion wasm service listening on http://${host}:${resolvedPort}`,
    );
  });
  return server;
}
