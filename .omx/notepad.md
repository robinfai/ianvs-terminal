...
[2026-04-22T03:08:50Z] Synced the terminal manual-matrix docs to the current local-host reality. The resize-lane runbook/playbook refinements remain in place, but the current host now proves `flutterm_no_proxy` is absent, `vttest` is blocked again, and fixed-port `flutter run -d macos --host-vmservice-port 49200` still cannot yield a verified foreground keyboard session because the app stays behind WeChat and this session lacks `System Events` accessibility plus screenshot permission. T-054 should now be read as `unsuitable local host`, and T-055 must continue off-machine.
